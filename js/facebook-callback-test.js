...
class FacebookCallbackHandler {
    ...
    handleAuthCode(code, state) {
        ...
        this.simulateTokenExchange(code, state);
    }
    
    simulateTokenExchange(code, state) {
        this.updateStatus('Exchanging authorization code for access token...', 'processing');
        
        setTimeout(() => {
            // Simulate successful token exchange
            const mockAccessToken = 'mock_access_token_' + Date.now();
            this.getUserInfo(mockAccessToken);
        }, 1500);
    }
    ...
    getUserInfo(accessToken) {
        ...
        if (this.isTestMode) {
            ...
            setTimeout(() => {
                ...
                this.handleUserData(mockUserData, accessToken);
            }, 1000);
        } else {
